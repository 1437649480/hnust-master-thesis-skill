...
class ThesisVerifier:
    ...
    def _verify_h2(self, para, text):
        ...
        if run:
            ...
            if run.font.size and check_emu(run.font.size, spec['size']):
                self.verify('字体', True, f'节标题"{short}" 小三号(15pt)')
            else:
                actual_pt = run.font.size / 12700 if run.font.size else None
                self.verify('字体', False, f'节标题"{short}" 字号应为小三号(15pt)',
                            f'实际: {actual_pt}pt' if actual_pt else '实际: 无字号')
    ...
    def _verify_h3(self, para, text):
        ...
        if run:
            ...
            if run.font.size and check_emu(run.font.size, spec['size']):
                self.verify('字体', True, f'条标题"{short}" 四号(14pt)')
            else:
                actual_pt = run.font.size / 12700 if run.font.size else None
                self.verify('字体', False, f'条标题"{short}" 字号应为四号(14pt)',
                            f'实际: {actual_pt}pt' if actual_pt else '实际: 无字号')
    ...
    def _verify_ref_title(self, para, text):
        ...
        if run:
            ...
            if run.font.size and check_emu(run.font.size, spec['size']):
                self.verify('字体', True, '参考文献标题 小四号(12pt)')
            else:
                actual_pt = run.font.size / 12700 if run.font.size else None
                self.verify('字体', False, '参考文献标题 字号应为小四号(12pt)',
                            f'实际: {actual_pt}pt' if actual_pt else '实际: 无字号')
    ...
    def _verify_ref_entry(self, para, text):
        ...
        if run:
            ...
            if run.font.size and check_emu(run.font.size, spec['size']):
                self.verify('字体', True, f'参考文献条目"{short}" 小五号(9pt)')
            else:
                actual_pt = run.font.size / 12700 if run.font.size else None
                self.verify('字体', False, f'参考文献条目"{short}" 字号应为小五号(9pt)',
                            f'实际: {actual_pt}pt' if actual_pt else '实际: 无字号')
