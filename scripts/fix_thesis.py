# -*- coding: utf-8 -*-
"""
湖南科技大学硕士研究生学位论文格式修复脚本
基于《湖南科技大学研究生学位论文撰写规范》
用法: python fix_thesis.py <docx文件路径> [输出文件路径]
"""
...
# 页面设置: A4纸(210mm×297mm), 上30mm 下30mm 左25mm 右25mm, 页眉20mm 页脚20mm
PAGE_SPEC = {
    'width': Cm(21),        # A4宽度 210mm
    'height': Cm(29.7),     # A4高度 297mm
    'top': Cm(3.0),         # 上页边距 30mm
    'bottom': Cm(3.0),      # 下页边距 30mm
    'left': Cm(2.5),        # 左页边距 25mm
    'right': Cm(2.5),       # 右页边距 25mm
    'header': Cm(2.0),      # 页眉距边界 20mm
    'footer': Cm(2.0),      # 页脚距边界 20mm
}

# 页眉: 内容"湖南科技大学硕士学位论文"，五号宋体(10.5pt)居中
HEADER_SPEC = {
    'text': '湖南科技大学硕士学位论文',
    'font': '宋体',
    'size': Pt(10.5),       # 五号
...
# 段落格式规范
FONT_SPEC = {
    # 章标题: 第X章 ...  小2号宋体加粗(18pt), 居中, 上下各空一行(段前段后各12pt), 单倍行距
    'h1': {
        'font': '宋体', 'size': Pt(18), 'bold': True,
        'align': WD_ALIGN_PARAGRAPH.CENTER,
        'before': Pt(12), 'after': Pt(12),
        'line_spacing': 1.0,
    },
    # 节标题: X.Y ...  小3号宋体加粗(15pt), 顶格, 上下各空0.5行(段前段后各6pt), 单倍行距
    'h2': {
        'font': '宋体', 'size': Pt(15), 'bold': True,
        'align': WD_ALIGN_PARAGRAPH.LEFT,
        'before': Pt(6), 'after': Pt(6),
        'line_spacing': 1.0,
    },
    # 条标题: X.Y.Z ...  4号宋体加粗(14pt), 顶格, 上下各空0.5行(段前段后各6pt), 单倍行距
    'h3': {
    ...
    # 款/项标题: (1) (一) 等  小4号宋体加粗(12pt), 上下不空, 单倍行距, 首行缩进2字符
    'h4': {
        'font': '宋体', 'size': Pt(12), 'bold': True,
        'align': WD_ALIGN_PARAGRAPH.LEFT,
        'before': Pt(0), 'after': Pt(0),
        'line_spacing': 1.0, 'first_line_indent': Pt(24),
    },
    # 正文: 小4号宋体(12pt), 1.25倍行距, 首行缩进2字符(24pt)
    'body': {
    ...
    # 参考文献条目: 小5号宋体(9pt), 顶格
    'ref_entry': {
        'font': '宋体', 'size': Pt(9), 'bold': False,
        'align': WD_ALIGN_PARAGRAPH.LEFT,
    },
    # 摘要标题: 小2号宋体加粗(18pt), 居中
    'abstract_title': {
        'font': '宋体', 'size': Pt(18), 'bold': True,
        'align': WD_ALIGN_PARAGRAPH.CENTER,
    },
    # ABSTRACT标题: Times New Roman加粗小2号(18pt), 居中
    'en_abstract_title': {
    ...
    # Keywords标签: Times New Roman加粗小4号(12pt)
    'en_kw_label': {
        'font': 'Times New Roman', 'size': Pt(12), 'bold': True,
    },
    # Keywords词条: Times New Roman小4号(12pt)
    'en_kw_content': {
        'font': 'Times New Roman', 'size': Pt(12), 'bold': False,
    },
    # 致谢标题: 小2号宋体加粗(18pt), 居中
    'thanks_title': {
        'font': '宋体', 'size': Pt(18), 'bold': True,
        'align': WD_ALIGN_PARAGRAPH.CENTER,
    },
    # 附录标题: 小2号宋体加粗(18pt), 居中 (与章标题同级)
    'appendix_title': {
        'font': '宋体', 'size': Pt(18), 'bold': True,
        'align': WD_ALIGN_PARAGRAPH.CENTER,
        'before': Pt(12), 'after': Pt(12),
...
class ThesisFixer:
    ...
    def fix_page_setup(self):
        """修复所有节的页面设置为A4纸，上30mm 下30mm 左25mm 右25mm，页眉20mm 页脚20mm"""
        fixed_count = 0
    ...
    def fix_paragraphs(self):
        ...
        for i, para in enumerate(self.doc.paragraphs):
            ...
            # 10. 条标题: X.Y.Z ... (三级及以上，先于节标题匹配，避免被节标题误匹配)
            if match_h3(text):
    ...
    def _fix_en_keywords(self, para, text):
        """
        修复英文关键词行格式。
        "Keywords"标签: Times New Roman加粗小4号(12pt)
        词条: Times New Roman小4号(12pt)
        """
        for run in para.runs:
    ...
    def fix_table_cells(self):
        ...
        for ti, table in enumerate(self.doc.tables):
            ...
            if ti == 0:
                ...
                if first_cell in ('题目', '作者', '学院', '学科专业', '研究方向', '研究生', '指导教师', '论文编号'):
                    continue
    ...
    def run_all(self):
        """执行所有修复项并输出统计"""
        print(f"\n{'=' * 60}")
        print(f"  湖南科技大学硕士学位论文格式修复")
        print(f"  文件: {self.filepath}")
        print(f"{'=' * 60}")

        # 1. 页面设置
        self.fix_page_setup()
        print(f"\n  [页面] 页面设置: 修复 {self.stats['page_setup']} 个节")

        # 2. 页眉
        self.fix_header()
        print(f"  [页眉] 页眉: 修复 {self.stats['header']} 个节")
        ...
        # 4. 引用标注
        self.fix_citations()
        print(f"\n  [引用] 引用标注上标: 修复 {self.stats['citation']} 处")

        # 5. 表内文字
        self.fix_table_cells()
        print(f"  [表格] 表内文字格式: 修复 {self.stats.get('table_cell', 0)} 处")

        # 6. 分页符
        self.fix_page_breaks()
        print(f"  [分页] 分页符: 插入 {self.stats.get('page_break', 0)} 处")
...
def main():
    if len(sys.argv) < 2:
        print("用法: python fix_thesis.py <docx文件路径> [输出文件路径]")
        print("示例: python fix_thesis.py 论文.docx")
        print("      python fix_thesis.py 论文.docx 论文_fixed.docx")
        sys.exit(1)
