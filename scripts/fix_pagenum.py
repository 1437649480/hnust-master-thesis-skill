...
def setup_header_for_section(section):
    """
    为单个节设置页眉内容。
    断开 is_linked_to_previous，清空原页眉，重新构建。
    页眉内容：湖南科技大学硕士学位论文
    """
    ...
    # 重新获取 header（断开链接后可能创建了新的 header 部件）
    header = section.header

    # 删除所有现有段落
    header_element = header._element
    for p in header_element.findall(qn('w:p')):
        header_element.remove(p)
    for tbl in header_element.findall(qn('w:tbl')):
        header_element.remove(tbl)
    ...
    # 添加页眉内容
    new_para.append(_make_text_run('湖南科技大学硕士学位论文'))

    # 将新段落添加到页眉
    header_element.append(new_para)
...
def setup_page_margins(section):
    ...
    # 设置页面边距（单位：twips，1mm = 56.7 twips）
    # 上边距: 30mm = 1701 twips
    ...
    # 页眉距离: 20mm = 1134 twips
    pgMar.set(qn('w:header'), '1134')
    # 页脚距离: 20mm = 1134 twips
    pgMar.set(qn('w:footer'), '1134')
...
def main():
    ...
    # ─── 第二步：重新打开文档，设置页脚、页眉和页面边距 ───
    print("【第二步】设置页脚、页眉和页面边距")
    ...
    for i, section in enumerate(sections):
        ...
        # 设置页眉内容
        setup_header_for_section(section)
        print(f"    页眉已设置为 \"湖南科技大学硕士学位论文\"")

        # 设置页面边距
        setup_page_margins(section)
        print(f"    页面边距已设置 (上30mm 下30mm 左25mm 右25mm 页眉20mm 页脚20mm)")
